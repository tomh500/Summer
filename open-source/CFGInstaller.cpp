...
void AppendIfMissing(const path& filePath, const string& lineToAdd)
{
    ...
    if (content.find(lineToAdd) == string::npos) {
        ...
        out << lineToAdd << "\n";
        MessageBoxW(nullptr, L"已成功添加 exec Summer/Setup 到 autoexec.cfg", L"完成", MB_OK | MB_ICONINFORMATION);
    }
    else {
        MessageBoxW(nullptr, L"autoexec.cfg 已包含 exec Summer/Setup，无需添加", L"信息", MB_OK | MB_ICONINFORMATION);
    }
}
...
extern "C" __declspec(dllexport) int __cdecl  CFGInstaller_Asul()
{
    ...
    path CFGRootPath = path(exePath).parent_path(); 
    CFGRootPath = CFGRootPath.parent_path();        
    CFGRootPath = CFGRootPath.parent_path();      

    path srcRoot = CFGRootPath / L"src" / L"CS2" / L"resources";

    // 文件复制操作，一旦失败立即返回 1
    if (!CopyFile(
        (srcRoot / L"Summer_Installed.cfg").wstring(),
        (CFGRootPath.parent_path().parent_path().parent_path() / L"cfg" / L"Summer_Installed.cfg").wstring()
    )) return 1;
    ...
    if (!CopyFile(
        (srcRoot / L"gamestate_integration_Summer.cfg").wstring(),
        (CFGRootPath.parent_path() / L"gamestate_integration_Summer.cfg").wstring()
    )) return 1;

    //在Sounds下创建目录
    path soundsDir = CFGRootPath.parent_path().parent_path() / L"sounds" / L"Summer";
   ...
   // if (MessageBox(NULL, L"是：游戏锁帧 < 540\n否：游戏锁帧 > 540\n", L"标题", MB_ICONINFORMATION | MB_YESNO) == IDYES) { StartAppsNew(RootPath / L"src" / L"CS2" / L"main" / L"DM" / L"Tools" / L"Ticker" / L"GenAll_Normal.bat", L"", true); }
   // else { StartAppsNew(RootPath / L"src" / L"CS2" / L"Tools" / L"Ticker" / L"GenAll_High.bat", L"", true); };
    ...
    if (response == IDYES) {
        ...
        try {
            ...
            AppendIfMissing(autoexecPath, "exec Summer/Setup");
        }
        catch (...) {
...
int CFGInstaller()
{
  ...
  //  if (MessageBox(NULL, L"是：游戏锁帧 < 540\n否：游戏锁帧 > 540\n", L"标题", MB_ICONINFORMATION | MB_YESNO) == IDYES) { StartAppsNew(RootPath/L"src"/L"CS2" / L"main" / L"DM" / L"Tools" / L"Ticker" / L"GenAll_Normal.bat", L"", true); }
    ...
    if (result == IDYES) {
        StartAppsNew(RootPath / L"src" / L"CS2" / "lib" / "Asul_Editor.exe", L"-asulink ..\\..\\..\\fastconfig.asulink", false);
    }
