 ...
 - killing_sound.json
 - 道具轮盘站位图.exe
 并声明你已完整阅读，在提问时附带截图或视频，确保 CFG 开启 Debug 模式，否则你的提问将被忽略。
 ...
 注意：Linux 路径区分大小写，部分模块将不可用  
 若遇问题，请到 Discord 频道反馈并附信息  

 ## 关于半条命与 CS1.6
 - 用户空间：`DearMoments/src/half-life/Userspace.cfg`  
 - 启动项：`+exec DearMoments/Setup_HL.cfg`  
 注意：此为 CS1.6 使用方式，CS2 用户请继续阅读下文

 ## GSI-击杀音效替换使用方式
 请阅读：`Userspace/gsi/killing_sound/killing_sound.json`
 ...
 ## 如何卸载 CFG
 1. 删除 `autoexec.cfg` 中的 `exec MoClient/setup`  
 ...
 ### 为什么锁帧
 在 UserValue.cfg 设置，但帧率不得超过 1009，确保正确构建 ticker
 ...
 ## 关于倒卖
 可自由倒卖，只要保留著作权。是否被骂不负责。欢迎尝试破解正版验证。

 ## 关于赞助与定制
 - 使用免费  
 - 可自愿赞助  
 - 定制需私信 B站 Luotiany1_Mar 或 Discord，含商业条款
 ...
 **Q：如何使用自动身法？**  
 ...
 2. 编辑四个配置文件：UserKeyBinds、UserKZMode、UserSetting、UserValue  
 ...
 ```
 若出现 `bind key +DM_bind_radialradio`，请将 `key` 改为实际按键

 ### 设置输出方式示例（UserSetting）
 ```
 ...
 ---  
 **最后修订 by Luotiany1_Mar at Jul 15th 2025**
