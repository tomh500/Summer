...
int main() {
    ...
    outfile << "exec DearMoments/src/main/Tools/define_mousexy;\n";
    outfile.close();

    cout << "文件写入完成: " << filePath << endl;

  try {
        // 1. src/resources/DearMoments_Installed.cfg -> ../../../cfg
        fs::path src1 = "src/resources/DearMoments_Installed.cfg";
        ...
        // 2. src/resources/gamestate_integration_dearmoments.cfg -> ..
        fs::path src2 = "src/resources/gamestate_integration_dearmoments.cfg";
        ...
        // 3. src/resources/keybindings_english.txt 和 keybindings_english.txt -> ../../resource
        fs::path dst3 = fs::path("../../resource");
        fs::create_directories(dst3);

        fs::path src3a = "src/resources/keybindings_english.txt";
        fs::copy_file(src3a, dst3 / src3a.filename(), fs::copy_options::overwrite_existing);
        cout << "复制完成: " << src3a << " -> " << dst3 << endl;

        fs::path src3b = "src/resources/keybindings_schinese.txt";  // 当前目录下的文件
    ...
    fs::path ticker_dir = "src/main/Tools/Ticker";
    if (ans == "y" || ans == "yes") {
